...
现场核验通过后，维护人员还需判断：**临时封闭任意一块非端点触觉砖，是否会切断入口到服务点**。点击侧栏“⚠ 单点中断分析”启动：

- **风险格（橙色标记）**：逐格试移除每块非端点触觉砖并重新做四邻接洪水遍历，移除后令两端点失联的格即为风险格；带支路时支路砖不会误报。
- **点选预览**：点选橙色风险格后，以半透明橙色叠加显示**服务点一侧不可达区域**（封闭该格后入口分量之外的全部触觉砖，含服务点）；再次点选同一格取消。
- **确定性**：候选格按格索引升序试移除，风险格与影响区均按格索引升序输出，重复计算结果完全一致。
- **联动规则**：分析只在原核验通过时执行；路线编辑、撤销、清空或载入示例后立即清除旧选择并按当前棋盘重算；核验未通过时展示“先修复原核验问题”的反馈，且不留任何过期覆盖层。分析进行中点选橙色格为预览（不绘制），如需编辑橙色格本身请先关闭分析。
...
```
src/
  domain/grid.ts          # 纯领域逻辑：四邻接遍历、确定性 BFS、核验、割点分析、编辑
  domain/grid.test.ts     # Vitest 判据（32 个用例）
  components/BoardView.vue# SVG 栅格：绘制、问题格红标、路径高亮、风险格橙标与影响区
  App.vue                 # 工具、撤销栈、统计与结论面板、单点中断分析三阶段状态
tests/e2e/board.spec.ts        # Playwright：绘制 / 撤销 / 高亮 / 孤岛保红
tests/e2e/cut-analysis.spec.ts # Playwright：启动分析 / 点选预览 / 编辑刷新 / 未通过无过期覆盖层
...
- `analyzeCutCells(state)`：割点分析。逐格试移除非端点触觉砖后重跑洪水遍历，两端点失联即记为风险格；受影响区域取移除后入口分量之外的触觉砖（服务点一侧）。候选与输出均按格索引升序，结果确定。
